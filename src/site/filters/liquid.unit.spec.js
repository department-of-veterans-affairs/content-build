...
describe('filterClinicalHealthServicesByTypeOfCare', () => {
  const serviceType = 'Social programs and services';
  const clinicalHealthSocialPrograms = {
    entityId: '1',
    entityType: 'node',
    entityUrl: {
      path:
        '/lovell-federal-health-care-tricare/health-services/social-programs',
    },
    fieldBody: {
      processed:
        '<html><head></head><body><p>Description Here.</p></body></html>',
    },
    ...
    fieldServiceNameAndDescripti: {
      entity: {
        ...
        fieldAlsoKnownAs: 'Social Programs and Services',
        fieldCommonlyTreatedCondition: null,
        name: 'Social Programs and Services',
        description: {
          processed: '<html><head></head><body></body></html>',
        },
        fieldTricareDescription: 'Description Here',
        fieldServiceTypeOfCare: serviceType,
      },
    },
  };

  const clinicalHealthEmptyString = {
    entityId: '2',
    entityType: 'node',
    entityUrl: {
      path: '/lovell-federal-health-care-tricare/health-services/nothing',
    },
    fieldBody: {
      processed:
        '<html><head></head><body><p>Description Here.</p></body></html>',
  ...
  const clinicalHealthNullCare = {
    ...
    fieldServiceNameAndDescripti: {
      entity: {
        ...
        fieldTricareDescription: 'Description Here',
        fieldServiceTypeOfCare: null,
      },
    },
  };

  it('removes clinical services without a type of care', () => {
    const filteredResults = liquid.filters.filterClinicalHealthServicesByTypeOfCare(
      [
        clinicalHealthSocialPrograms,
        clinicalHealthEmptyString,
        clinicalHealthNullCare,
      ],
      serviceType,
    );

    expect(filteredResults.length).to.eq(1);
    expect(
      filteredResults[0].fieldServiceNameAndDescripti.entity
        .fieldServiceTypeOfCare,
    ).to.eq(serviceType);
  });
});
