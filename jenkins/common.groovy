...
def checkForBrokenLinks(String buildLogPath, String envName, Boolean contentOnlyBuild) {
  ...
  if (fileExists(csvFile)) {
    sh "rm /content-build/${csvFileName}"
  }

  // Output a csv file with the broken links
  sh "cd /content-build && jenkins/glean-broken-links.sh ${buildLogPath} ${csvFileName}"
  if (fileExists(csvFile)) {
    ...
    def linkCount = sh(returnStdout: true, script: "cd /content-build && wc -l ${csvFileName} | cut -d ' ' -f1") as Integer
...
def build(String ref, dockerContainer, String assetSource, String envName, Boolean validateContent, Boolean useCache, Boolean contentOnlyBuild) {
  ...
  stage("Build") {
    ...
    try {
      ...
      for (int i=0; i<VAGOV_BUILDTYPES.size(); i++) {
        ...
        builds[envName] = {
          try {
            build(ref, dockerContainer, assetSource, envName, false, false, contentOnlyBuild)
            ...
            if (!contentOnlyBuild) {
              ...
              build(ref, dockerContainer, assetSource, envName, false, true, contentOnlyBuild)
