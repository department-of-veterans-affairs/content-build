...
set -o pipefail
npm --no-color run build -- --checkBrokenLinks="$checkBrokenLinks"  --buildtype="$envName" --asset-source="$assetSource" --drupal-address="$drupalAddress" "$pullDrupal" 2>&1 | tee "$buildLog"
