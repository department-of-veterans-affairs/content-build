...
if (useVetsWebsite) {
  ...
  runCommandSync('yarn && yarn build --port 3001 --pull-drupal --omitdebug'); // run content-build build
  runCommandSync('cp -r  ./cache ../vets-website/'); // move content-build .cache into vets-website
  runCommandSync('cd ../vets-website && yarn build --omitdebug '); // run vets-website build
}
