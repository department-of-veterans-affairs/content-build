      ...
      try {
        parallel (
          ...
          "check-broken-links": {
            ...
            sh "docker-compose -p check-broken-links run --rm --entrypoint=npm -e BABEL_ENV=test -e BUILDTYPE=vagovstaging content-build --no-color run fetch-drupal-cache --buildtype=vagovstaging && run build --validateContent --buildtype=vagovstaging --drupal-fail-fast"
          },
        )
      } catch (error) {
